...
- Показывает содержимое плейлиста и даёт отметить нужные ролики до скачивания
...
- Ускоряет закачку с прямых доменов через aria2c, а фрагментированное видео - параллельными кусками
- Пишет живые эфиры с начала трансляции и умеет ждать анонсированный стрим
- Проигрывает готовый файл прямо на странице, отдаёт его на телефон по QR-коду и качает пачкой
- Держит публичную доску обратной связи - анонимную, без регистрации и без базы данных
...
Настройки, которым не место в git (прокси и пароль администратора доски обратной связи), передаются переменными окружения через `.env` - образец лежит в [`.env.example`](.env.example):
