...
Качалка - это веб-интерфейс для скачивания видео и аудио с разных сайтов. Внутри работает yt-dlp, а Качалка добавляет к нему удобную страницу, очередь загрузок, выбор роликов плейлиста, перевод озвучки, обход блокировок, доску обратной связи и кучу мелких удобств.
...
- [Первый запуск](tutorials/01-pervyy-zapusk.md) - от одной команды `docker run` до первого скачанного файла.
...
- [Включить обратную связь](how-to/09-obratnaya-svyaz.md)

## Справочник

Точные цифры и таблицы, без объяснений "зачем".

- [Параметры конфигурации](reference/01-config-parametry.md) - включая переменные окружения из `.env`
- [Формат ответа ?jobs](reference/02-api-jobs-endpoint.md) - и остальные точки входа: `?playlist`, `?feedback`, `?cron`
