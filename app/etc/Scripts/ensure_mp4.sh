...
# Строка в лог обязательна: тот же случай бывает и у оборванной склейки, и без
# отметки немой файл уезжал к человеку молча.
if [ -z "$acodec" ]; then
    aargs="-an"
    echo "[mp4] предупреждение: в файле нет аудиодорожки"
